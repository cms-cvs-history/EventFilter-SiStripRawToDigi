...
void SiStripRawToClustersLazyUnpacker::fill(uint32_t& iregion) {
  ...
  //If det has already been unpacked, return copy of requested det.
  if (unpacked()[iregion]) return;

  //Get region cabling and record
  const SiStripRegionCabling::RegionMap& rmap = (*regions_)[iregion];
  std::vector< DetSet >& rrecord = record()[iregion].second;

  //Reserve record
  rrecord.reserve(rmap.size());
  ...
  //record region as unpacked
  unpacked()[iregion] = true;
}
