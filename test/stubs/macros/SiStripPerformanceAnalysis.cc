...
void SiStripPerformanceAnalysis::timing() {
  ...
  for (unsigned int ievent=0;ievent<tree_->GetEntries();ievent++) {
    ...
    const double fraction = (100.*nunpackedchans_)/nchans_ : constants::invalid;
...
void SiStripPerformanceAnalysis::electron(unsigned int bit) {
  ...
  for (unsigned int ievent=0;ievent<tree_->GetEntries();ievent++) {
    ...
    for (unsigned int i=0;i<data_->mc().size();i++) {     
 
      if (!abs(ipart->pid()) == 11 || fabs(data_->mc()[i]->eta()) > constants::etaCut) continue;
      ...
      bool matched = (data_->electrons().size() &&  delta < constants::dRelec) ? true : false;
